...
const StaffDashboard = () => {
  ...
  useEffect(() => {
    const fetchMemberData = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`http://localhost:5000/api/members/${selectedMemberId}`);
        ...
        if (response.ok) {
          ...
          setProfilePictureURL(data.profilePic ? `http://localhost:5000/api/members/profile-pic/${data.profilePic}` : '');
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      profilePicture ? formData.append('file', profilePicture) : formData.append('file', '');
      ...
      const url = selectedMemberId 
        ? `http://localhost:5000/api/members/${selectedMemberId}`
    ...
    return (
      <div className="p-8">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h2 className="text-xl font-bold mb-4">Edit Existing Staff</h2>
            <select
    ...
    <form onSubmit={handleSubmit} className="min-h-screen bg-gray-100 p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-900">
          {selectedMemberId ? 'Edit Staff Member' : 'Add New Staff Member'}
        </h1>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Staff ID</h2>
        <input
      ...
      >
        {isLoading ? 'Saving...' : (selectedMemberId ? 'Update Member' : 'Add Member')}
      </button>
    </form>
  );
};
