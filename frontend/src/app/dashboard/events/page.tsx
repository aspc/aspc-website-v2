...
const AdminEventsDashboard = () => {
                ...
                const eventsWithStats = await Promise.all(
                    eventsData.map(async (event) => {
                        try {
                            ...
                            console.error(
                                `Failed to fetch stats for event ${event._id}`,
                                error
                            );
                        }
                        return event;
    ...
    useEffect(() => {
        ...
        result.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'rating':
                    compareValue =
                        (a.stats?.averageOverall || 0) -
                        (b.stats?.averageOverall || 0);
                    break;
                case 'reviews':
                    compareValue =
                        (a.stats?.totalReviews || 0) -
                        (b.stats?.totalReviews || 0);
                    break;
                case 'wouldRepeat':
                    compareValue =
                        (a.stats?.averageWouldRepeat || 0) -
                        (b.stats?.averageWouldRepeat || 0);
                    break;
            }
    ...
    const calculateOverallAnalytics = (eventsData: EventWithStats[]) => {
        const eventsWithReviews = eventsData.filter(
            (e) => e.stats && e.stats.totalReviews > 0
        );
        ...
        const totalReviews = eventsWithReviews.reduce(
            (sum, e) => sum + (e.stats?.totalReviews || 0),
            0
        );
        const totalRating = eventsWithReviews.reduce(
            (sum, e) =>
                sum +
                (e.stats?.averageOverall || 0) * (e.stats?.totalReviews || 0),
            0
        );
        const totalWouldRepeat = eventsWithReviews.reduce(
            (sum, e) =>
                sum +
                (e.stats?.averageWouldRepeat || 0) *
                    (e.stats?.totalReviews || 0),
            0
        );
        ...
        const highestRated = [...eventsWithReviews].sort(
            (a, b) =>
                (b.stats?.averageOverall || 0) - (a.stats?.averageOverall || 0)
        )[0];
            ...
            <div className="min-h-screen bg-gray-100 flex items-center justify-center">
                <div className="bg-white p-8 rounded-lg shadow-md">
                    ...
                    <p className="text-gray-700">
                        You must have admin access to see this page.
                    </p>
                </div>
            </div>
        );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <StarRating
                                rating={Math.round(
                                    overallAnalytics.averageWouldRepeat
                                )}
                            />
                        </div>
                    </div>
                                            ...
                                            {event.stats?.averageOverall ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-medium">
                                                        {event.stats.averageOverall.toFixed(
                                                            1
                                                        )}
                                                    </span>
                                                </div>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {event.stats?.averageWouldRepeat ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <span className="text-sm font-medium">
                                                        {event.stats.averageWouldRepeat.toFixed(
                                                            1
                                                        )}
                                                    </span>
                                                </div>
