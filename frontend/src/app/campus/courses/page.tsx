...
const schoolData = {
    PO: { name: 'Pomona', bgColor: 'bg-blue-400', buttonColor: 'bg-blue-100 text-blue-800 border-blue-300', textColor: 'text-blue-800' },
    CM: { name: 'Claremont McKenna', bgColor: 'bg-red-400', buttonColor: 'bg-red-100 text-red-800 border-red-300', textColor: 'text-red-800' },
    HM: { name: 'Harvey Mudd', bgColor: 'bg-yellow-300', buttonColor: 'bg-yellow-100 text-yellow-800 border-yellow-300', textColor: 'text-yellow-800' },
    SC: { name: 'Scripps', bgColor: 'bg-green-200', buttonColor: 'bg-green-100 text-green-800 border-green-300', textColor: 'text-green-800' },
    PZ: { name: 'Pitzer', bgColor: 'bg-orange-300', buttonColor: 'bg-orange-100 text-orange-800 border-orange-300', textColor: 'text-orange-800' },
};
...
const CourseSearchComponent = () => {
    ...
    const fetchInstructors = useCallback(async (ids: number[]) => {
        try {
            ...
            const response = await axios.get<Instructor[]>(`${process.env.BACKEND_LINK}/api/instructors/bulk`, {
            ...
            setInstructorCache(prev => ({
                ...prev,
                ...response.data.reduce((acc, instructor) => {
                    acc[instructor.id] = instructor;
                    return acc;
                }, {} as Record<number, Instructor>)
            }));
        } catch (err) {
            if (!axios.isCancel(err)) console.error('Error fetching instructors:', err);
        }
    }, []);

    const performSearch = useCallback(async (term: string, schools: Record<SchoolKey, boolean>, page: number, itemLimit: number) => {
        ...
        if (cancelTokenSourceRef.current) cancelTokenSourceRef.current.cancel('Operation canceled due to new request');
        ...
        try {
            ...
            if (coursesData.length > 0) {
                const instructorIds = coursesData.flatMap(course => course.all_instructor_ids || []);
                if (instructorIds.length > 0) fetchInstructors(instructorIds);
            }
        } catch (err) {
    ...
    const handleSchoolToggle = (school: SchoolKey) => {
        setSelectedSchools(prev => {
            ...
            let newState: Record<SchoolKey, boolean>;
            if (allSelected) {
                newState = { PO: false, CM: false, HM: false, SC: false, PZ: false };
                newState[school] = true;
            } else {
                newState = { ...prev, [school]: !prev[school] };
                if (!Object.values(newState).some(Boolean)) newState = { PO: true, CM: true, HM: true, SC: true, PZ: true };
            }
        ...
        <div className="min-h-screen bg-gray-100 text-gray-900">
            <div className="max-w-6xl mx-auto p-4 md:p-6">
                <div className="bg-gray-50 rounded-lg shadow-md p-6 mb-6">
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6">5C Course Search</h1>

                    <div className="flex flex-col md:flex-row gap-4 mb-4">
                        <div className="flex-1">
                            <label htmlFor="search-term" className="block text-sm font-medium text-gray-700 mb-1">Course Name or Code</label>
                            <input
                                id="search-term"
                                type="text"
                                ...
                                <p className="text-gray-600 text-sm">
                                    {pagination ? (
                                        <>
                                            ...
                                            {Math.min(currentPage * limit, pagination.totalCount)} of {pagination.totalCount} results
                                        </>
                                    ) : (
                                        <>
                            ...
                            {pagination && (
                                <div className="flex justify-center items-center gap-2 mt-8 pb-4">
                                    ...
                                    <div className="flex gap-1">
                                        {currentPage > 2 && (
                                            <>
                                                <button onClick={() => handlePageChange(1)} className="px-3 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors text-sm">1</button>
                                        ...
                                        {currentPage < pagination.totalPages - 1 && (
                                            <>
                                                {currentPage < pagination.totalPages - 2 && <span className="px-2 py-2">...</span>}
                                                <button onClick={() => handlePageChange(pagination.totalPages)} className="px-3 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors text-sm">{pagination.totalPages}</button>
                                            </>
                                        )}
                                    </div>
...
// CourseCardComponent remains unchanged
const CourseCardComponent = ({ course, schoolCode, instructorCache }: Omit<CourseCardProps, 'onInstructorLoad'>) => {
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow relative">
            ...
            <div className="p-6">
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="text-xl font-bold text-gray-800 mb-1">{course.code}: {course.name}</h3>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    {course.department_names?.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                                {course.department_names.map((dept, idx) => <span key={idx} className="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">{dept}</span>)}
                            </div>
                        </div>
                    )}
                    {course.requirement_names?.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                                {course.requirement_names.map((req, idx) => <span key={idx} className="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">{req}</span>)}
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="flex justify-between">
                        ...
                        {course.all_instructor_ids?.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-1">
                                {course.all_instructor_ids
                                    ...
                                    .filter(ins => ins?.name)
                                    .map(ins => (
                                        <div key={ins!.id} className={`${schoolData[schoolCode].bgColor} bg-opacity-20 px-2 py-1 rounded-md border ${schoolData[schoolCode].bgColor} border-opacity-30`}>
                                            <a href={`/campus/instructors/${ins!.id}`} className={`${schoolData[schoolCode].textColor} text-sm font-medium`}>{ins!.name}</a>
                                        </div>
                                    ))}
                            </div>
                        )}
                    ...
                    <div className="mt-4">
                        ...
                        >
                            ...
                            ? `View ${course.review_count} ${course.review_count === 1 ? 'Course Review' : 'Course Reviews'}`
